...
a = pmax(2,ceiling(int_times$length/width))
ks.test(a,b)
...
ggplot(times,aes(x=length,fill=code)) +
    ...
    facet_grid(type~.) +
    xlim(0,25) + xlab('length (s)')
ggsave(paste("david_cont_vs_int",Sys.Date(),".pdf",sep=""))
...
ggsave(paste("david_quant_cont_vs_int",Sys.Date(),".pdf",sep=""))
